...
export default function (pi: ExtensionAPI) {
	const dangerousCommandRules: Rule[] = [
		...
		{ name: "disk/filesystem tooling", pattern: /(^|\s)(mkfs|fdisk|parted|diskutil\s+eraseDisk)\b/i },
	...
	const sensitivePathPatterns: Rule[] = [
		...
		{ name: "SSH config/keys", pattern: /(^|\/)\.ssh(\/|$)|id_rsa|id_ed25519|known_hosts|authorized_keys/i },
		{ name: "npm/yarn auth", pattern: /(^|\/)\.npmrc$|(^|\/)\.yarnrc(\.yml)?$/i },
		{ name: "system config", pattern: /^\/etc\//i },
		{ name: "shell profile", pattern: /(^|\/)\.(zshrc|bashrc|bash_profile|profile|zprofile)$/i },
		{ name: "credentials", pattern: /secret|token|credential|private[_-]?key/i },
	];

	pi.on("tool_call", async (event, ctx) => {
		if (event.toolName === "bash") {
			const command = String(event.input.command ?? "");
			const matches = dangerousCommandRules.filter((r) => r.pattern.test(command));
			if (matches.length === 0) return undefined;

			if (!ctx.hasUI) {
				return {
					block: true,
					reason: `Blocked dangerous command (no UI): ${matches.map((m) => m.name).join(", ")}`,
				};
			}
		...
		if (event.toolName === "write" || event.toolName === "edit") {
			...
			const matchedRules = matches.length > 0 ? matches.map((m) => m.name).join(", ") : "none";
